<script>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const vendorRes = await fetch(`${STRAPI}/api/vendors`, {
        method: 'POST',
        headers: {
      ...
      // Step 4: Set SSR cookies and localStorage
      if (browser) {
  ...
  <aside class="brand-panel">
    ...
    <div class="brand-body">
      ...
      <ul class="brand-perks">
        <li><span class="perk-icon">📍</span><span>List products in your local market</span></li>
        <li><span class="perk-icon">📦</span><span>Manage your inventory easily</span></li>
        <li><span class="perk-icon">📈</span><span>Reach buyers across the country</span></li>
      </ul>
    </div>
        ...
        <div class="form-fields">
          ...
          <div class="field">
            ...
            <input id="name" type="text" bind:value={form.name}
              placeholder="e.g. Nakato Fresh Produce" autocomplete="name" />
          </div>
          ...
          <div class="field">
            ...
            <div class="input-with-prefix">
              ...
              <input id="contact" type="tel" bind:value={form.contact}
                placeholder="+256 700 123 456" autocomplete="tel" />
            </div>
            ...
            <div class="field">
              ...
              <div class="input-with-toggle">
                <input id="password" type={showPassword ? 'text' : 'password'}
                  bind:value={form.password} placeholder="Min. 6 characters"
            ...
            <div class="field">
              ...
              <div class="input-with-toggle">
                <input id="confirm" type={showConfirm ? 'text' : 'password'}
                  bind:value={form.confirmPassword} placeholder="Re-enter password"
                  autocomplete="new-password" />
                <button type="button" class="toggle-btn"
...
<style>
  ...
  .brand-panel {
    width: 380px; flex-shrink: 0; background: #24803f; color: #fff;
  ...
  .brand-logo {
    ...
    font-size: 2.5rem; font-weight: 700; color: #fff; text-decoration: none;
  }
  ...
  .brand-sub { font-size: 0.95rem; line-height: 1.6; color: #a8d5b5; margin: 0 0 36px; }

  .brand-perks { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 14px; }

  .brand-perks li { display: flex; align-items: center; gap: 12px; font-size: 0.9rem; color: #d4edd9; }

  .perk-icon { font-size: 1.1rem; width: 30px; text-align: center; }

  .brand-footer { font-size: 0.85rem; color: #a8d5b5; margin: 0; }
  .brand-footer a { color: #fff; font-weight: 600; text-decoration: none; }
  .brand-footer a:hover { text-decoration: underline; }

  .form-panel {
    flex: 1; display: flex; align-items: flex-start; justify-content: center;
    padding: 60px 32px; overflow-y: auto; background: #f5f5f0;
  }
  ...
  .form-header h1 {
    ...
    color: #111; margin: 0 0 8px; letter-spacing: -0.02em;
  }
  ...
  .alert {
    ...
    padding: 12px 16px; border-radius: 10px; font-size: 0.875rem; margin-bottom: 24px;
  }
  ...
  label { font-size: 0.8rem; font-weight: 600; color: #333; text-transform: uppercase; letter-spacing: 0.06em; }

  input {
    width: 100%; padding: 12px 14px; border: 1.5px solid #ddd; border-radius: 10px;
    font-size: 0.95rem; font-family: 'DM Sans', sans-serif; background: #fff; color: #111;
    box-sizing: border-box; transition: border-color 0.15s, box-shadow 0.15s; -webkit-appearance: none;
  }

  input:focus { outline: none; border-color: #1a5c2e; box-shadow: 0 0 0 3px rgba(26,92,46,0.1); }
  ...
  .input-with-prefix:focus-within { border-color: #1a5c2e; box-shadow: 0 0 0 3px rgba(26,92,46,0.1); }

  .prefix { padding: 0 12px; font-size: 1rem; border-right: 1.5px solid #eee; line-height: 1; }

  .input-with-prefix input { border: none; border-radius: 0; box-shadow: none; flex: 1; }
  ...
  .bar { height: 4px; flex: 1; border-radius: 4px; background: #e5e7eb; transition: background 0.3s; }
  .bar.weak   { background: #ef4444; }
  .bar.ok     { background: #f59e0b; }
  .bar.strong { background: #22c55e; }

  .strength-label { font-size: 0.75rem; color: #777; min-width: 40px; }

  .next-steps {
    margin: 28px 0 24px; padding: 16px 20px; background: #f0f7f2;
    border-radius: 12px; border-left: 3px solid #1a5c2e;
  }

  .next-title {
    font-size: 0.8rem; font-weight: 600; color: #1a5c2e;
    text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 10px;
  }

  .next-steps ul { margin: 0; padding: 0 0 0 16px; display: flex; flex-direction: column; gap: 4px; }
  .next-steps li { font-size: 0.88rem; color: #444; }

  .submit-btn {
    width: 100%; padding: 15px 24px; background: #108132; color: #fff;
  ...
  .submit-btn:hover:not(:disabled) { background: #0e4307; transform: translateY(-1px); }
  ...
  .login-nudge { text-align: center; font-size: 0.875rem; color: #777; margin: 16px 0 0; }
  .login-nudge a { color: #1a5c2e; font-weight: 600; text-decoration: none; }
  .login-nudge a:hover { text-decoration: underline; }

  .success-state { text-align: center; padding: 60px 0; }

  .success-icon {
    width: 72px; height: 72px; background: #dcfce7; color: #16a34a;
  ...
  .success-state h2 {
    ...
    font-weight: 400; margin: 0 0 10px; color: #111;
  }

  .success-state p { color: #666; margin: 0 0 28px; }

  .progress-bar {
    height: 4px; background: #e5e7eb; border-radius: 4px;
    overflow: hidden; max-width: 260px; margin: 0 auto;
  }

  .progress-fill {
    height: 100%; background: #1a5c2e; border-radius: 4px;
  ...
  @media (max-width: 768px) {
    ...
    .brand-panel { width: 100%; height: auto; position: static; padding: 28px 24px; }
