...
export async function GET({ cookies, fetch }) {
  try {
    ...
    console.log('🔐 Auth verify - Cookies present:', { hasJWT: !!jwt, hasUser: !!userCookie });
    ...
    const verifyResponse = await fetch(`${STRAPI}/api/users/me`, {
    ...
    if (vendorCookie) {
      ...
      // Fetch vendor from Strapi if not in cookies
      console.log('🔄 Vendor not in cookies, fetching from Strapi...');
      const vendorRes = await fetch(
        `${STRAPI}/api/vendors?filters[users_permissions_user][id][$eq]=${userData.id}&populate=*`,
